...
TFCoefs = [3 2 2 3 2 3]';
TFCenters = xSamp(:,[1 25  75 115 160  185])';
...
for ii = 1:size(kernEstimate,1)
    for jj = 1:size(kernEstimate,2)
        ...
        G(ii,jj) = X1f(ii,jj)+ X2f(ii,jj)+ getKernelEstimate([X1f(ii,jj),X2f(ii,jj)],TFCoefs,TFCenters,typeTF,betaTF);
    end
end
...
surf(X1,X2,G,'EdgeColor','none','FaceAlpha',0.5,'FaceColor','green');
% plot3(xSamp(1,:), xSamp(2,:),ySamp, 'r--', 'LineWidth', 2, 'MarkerSize', 5);
