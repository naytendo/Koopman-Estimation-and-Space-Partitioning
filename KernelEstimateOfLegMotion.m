load('HumanTreadmillDataKoopman.mat')
load('HumanTreadmillGRF.mat')
hip = markerData(:,1:3);
knee = markerData(:,4:6);
...
xSamp = [theta1(1:5000)',theta2(1:5000)']; %input data
ySamp = Output_Q(1:5000,2); %output data
depth = 5;
numChild = 2;
...
for pp = 1:M
    for jj = 1:M
        ...
        Kf(pp,jj) = kernel(type,centers(jj,:),Fcenters(pp,:),beta);
    end
end
...
x1Range = minGrid(1):1:maxGrid(1);
x2Range = minGrid(2):1:maxGrid(2);
...
ylabel('$\theta^2$','fontsize',20,'interpreter','latex')
zlabel('$U_f^M G^2$','fontsize',20,'interpreter','latex')
set(gca,'FontSize',14);
view(20,65)
